using Test, StaticArrays, EllZeroTrendFiltering
...
@test minimize_wrt_x2(Q1 + p1 ∘ @SMatrix[0 1]) == QuadraticPolynomial(1,0,1)
@test Q1(1, 2) == 1.5
...
@test minimize_wrt_x2(Q2 + p2 ∘ @SMatrix[0 1]) == QuadraticPolynomial(1,-2,5)

# x^2 + 2x + 2
Q3 = QuadraticForm(@SMatrix([1.0 0; 0 0]), @SVector([2.0, 0]), 2.0)
p3 = QuadraticPolynomial(0.0, 0.0, 0.0)
@test minimize_wrt_x2(Q3  + p3 ∘ @SMatrix[0 1]) == QuadraticPolynomial(1.0,2.0,2.0)


# y^2 + 2y + 2
Q4 = QuadraticForm(@SMatrix([0.0 0; 0 1]), @SVector([0.0, 2]),2.0)
p4 = QuadraticPolynomial(0.0, 0.0, 0.0)
@test minimize_wrt_x2(Q4 + p4 ∘ @SMatrix[0 1]) == QuadraticPolynomial(0,0,1)
