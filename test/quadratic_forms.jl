...
Q = dev.QuadraticForm(@SMatrix([2.0 0; 0 1]), @SVector([0.0,0]), 1.0) +
dev.QuadraticForm(@SMatrix([2.0 1; 1 2]), @SVector([1.0,0]), 0.0)
@testset "Quadratic Forms" begin
    @test Q == dev.QuadraticForm(@SMatrix([4.0 1; 1 3]), @SVector([1.0,0]), 1.0)
    ...
    # x^2 + y^2 + 1
    Q1 = dev.QuadraticForm(@SMatrix([1.0 0; 0 0]), @SVector([0.0, 0]), 0.5)
