...
# piecewise linear approximation (I, Y), given the transition costs l
function compute_cost(l, I, Y)
    cost = 0.0;
    for k=1:length(I)-1
     cost += l[I[k], I[k+1]](Y[k], Y[k+1])
    end
    return cost
end
...
for (g, I, Y, t_subsampled) in ((g1, I1, Y1, t_ss1), (g2, I2, Y2, t_ss2), (g3, I3, Y3, t_ss3))#, (g4, I4, Y4))
    ...
    for i in 1:length(t_subsampled)-1
        for ip in (i+1):length(t_subsampled)
            ...
            @test ℓ_subsampled[i, ip] == l[t_i, t_ip]
        end
    end
end
...
# (y - 1)^2 + (y/2 + y'/2 - 2)^2
@test l[1,3].P == [1.25 0.25; 0.25 0.25]
@test l[1,3].q == [-4.0, -2.0]
@test l[1,3].r == 5.0
